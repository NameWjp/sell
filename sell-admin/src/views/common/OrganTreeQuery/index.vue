...
export default {
  ...
  computed: {
    organizationTree() {
      return this.$store.state.organization.organizationTree || [];
    },
  },
  created() {
    if (this.defaultExpandeLevel) {
      const keys = flattenTree(getSubTree(this.organizationTree, this.defaultExpandeLevel)).map(item => item.id);
