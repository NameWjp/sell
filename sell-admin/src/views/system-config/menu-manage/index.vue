  ...
  <div class="directory-layout">
    ...
    <div class="main-layout">
      <div class="page-control">
        <el-button v-permission="'menu:create'" type="primary" icon="el-icon-plus" @click="handleToEdit('add')">新增</el-button>
        <el-button v-permission="'menu:delete'" type="danger" icon="el-icon-delete" :disabled="!selectedRows.length" @click="handleDeleteByRows(selectedRows)">删除</el-button>
      </div>
      ...
      >
        ...
        >
          <template #default="{ row }">
            ...
            <el-button v-permission="'menu:update'" type="text" @click="handleToEdit('edit', row)">编辑</el-button>
            <el-button v-permission="'menu:delete'" type="text" @click="handleDeleteByRows([row])">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
...
import { getMenuList, deleteMenu, getMenuTree } from '@/api/menu';
...
export default {
  ...
  methods: {
    ...
    async queryList() {
      const { list, ...pagination } = await getMenuList(this.pagination, {
        ...this.queryForm,
    ...
    handleDeleteByRows(rows) {
      ...
      this.$confirmMsg({
        message: `是否删除名称为 ${names.join(',')} 的菜单`,
        onOk: async () => {
          const { code } = await deleteMenu(ids);
          if (code === 200) {
